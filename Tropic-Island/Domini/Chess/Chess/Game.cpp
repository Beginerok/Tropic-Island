...
void Game::Move_Up()
{
	if (ActiveY != 0 && Matrix[ActiveX][ActiveY - 1] == 0)
	{
...
void Game::Move_Down()
{
	if (ActiveY != 7 && Matrix[ActiveX][ActiveY + 1] == 0)
	{
		Matrix[ActiveX][ActiveY + 1] = Matrix[ActiveX][ActiveY] ;
		Matrix[ActiveX][ActiveY] = 0; 
		ChessY[Active-1] -= 0.2;
...
{
	if (ActiveX != 7 && Matrix[ActiveX+1][ActiveY] == 0)
	{
...
void Game::Move_Left()
{
	if (ActiveX != 0 && Matrix[ActiveX - 1][ActiveY] == 0)
	{
		Matrix[ActiveX - 1][ActiveY] = Matrix[ActiveX][ActiveY] ;
		Matrix[ActiveX][ActiveY] = 0;
		ChessX[Active-1] -= 0.2;
...
void Game::BeganActive()
{
	for(int i=0;i<8;i++)
		for (int j = 0; j < 8; j++)
		{
			if (Active != -1)
			{
				if (oldx - WindowsSDLApi_->point.x > 0 && oldx - WindowsSDLApi_->point.x > 50)
					Move_Right();
				if (oldx - WindowsSDLApi_->point.x < 0 && oldx - WindowsSDLApi_->point.x > -50)
					Move_Left();
				if (oldy - WindowsSDLApi_->point.y > 0 && oldy - WindowsSDLApi_->point.y > 40)
					Move_Up();
				if (oldy - WindowsSDLApi_->point.y < 0 && oldy - WindowsSDLApi_->point.y > -40)
					Move_Down();
			}
			if (WindowsSDLApi_->point.x > QuadCoorXleft[i][j] && WindowsSDLApi_->point.x < QuadCoorXright[i][j] &&
				WindowsSDLApi_->point.y > QuadCoorYup[i][j] && WindowsSDLApi_->point.y < QuadCoorYdown[i][j] && Matrix[i][j] != 0 && Active ==-1)
			{
				ActiveY = j;
				ActiveX = i;
				Active = Matrix[i][j];
